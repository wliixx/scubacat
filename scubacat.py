...
def is_fist_clenched(lm):
    konchiki = [8,12,16,20]
    wrist = lm[0]
    close = [close_points(lm[k], wrist, 0.25) for k in konchiki]
    return all(close)

# функция, проверяющая, открыта ли ладонь
def openhand(lm):
    wrist_konchiki =[(8,6),(12,10),(16,14),(20,18)]
    return all(lm[k].y < lm[b].y for k, b in wrist_konchiki)
